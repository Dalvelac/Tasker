...
export async function onRequestPatch(context: AppContext) {
  ...
  let nextDateForRecurrence: string | null | undefined;
  let nextRecurrenceType: string | null | undefined;
  ...
  if (body.recurrence_days !== undefined) {
    const recurrenceDays = optionalRecurrenceDays(body.recurrence_days);
    if (recurrenceDays === undefined) return error('Invalid recurrence days');
    updates.push('recurrence_days = ?');
    values.push(nextRecurrenceType !== undefined && nextRecurrenceType !== 'weekly' ? null : recurrenceDays);
  }

  if (nextRecurrenceType !== undefined && body.recurrence_days === undefined) {
    if (nextRecurrenceType === 'weekly') {
      const existing = await context.env.DB.prepare('SELECT date FROM tasks WHERE id = ?')
        .bind(id)
        .first<{ date: string | null }>();
      const recurrenceDate = nextDateForRecurrence !== undefined ? nextDateForRecurrence : existing?.date ?? null;

      updates.push('recurrence_days = ?');
      values.push(recurrenceDate ? String(getWeekday(recurrenceDate)) : null);
    } else {
      updates.push('recurrence_days = NULL');
    }
  }
